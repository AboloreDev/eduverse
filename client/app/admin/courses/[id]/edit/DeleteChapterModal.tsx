...
const DeleteChapterModal = ({
  ...
  const handleDelete = async () => {
    if (confirmValue.trim() !== chapterTitle.trim()) {
      toast.error("Chapter name does not match.");
      return;
    }

    try {
      const response = await deleteChapter({ courseId, chapterId }).unwrap();
      if (response.success) {
        toast.success(
          `Chapter "${chapterTitle}" deleted and position re-ordered successfully.`
        );
        setIsOpen(false);
      } else {
